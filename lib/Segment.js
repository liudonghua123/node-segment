...
Segment.prototype.doSegment = function (text, options) {
  ...
  // 判断是否是raw，即不去除换行，空格，保留原格式
  if (options.raw) {
    var rret = me.tokenizer.split(text, me.modules.tokenizer);
    rret = me.optimizer.doOptimize(rret, me.modules.optimizer);
    if (rret.length > 0) ret = ret.concat(rret);
  }
  else {
    // 将文本按照换行符分割成多段，并逐一分词
